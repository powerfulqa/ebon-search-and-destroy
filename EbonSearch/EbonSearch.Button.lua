...
local me = CreateFrame( "Button", "EbonSearchButton", UIParent, "SecureActionButtonTemplate,SecureHandlerShowHideTemplate" );
EbonSearch.Button = me;
...
function me.PlaySound ( AlertSound )
	...
	if ( EbonSearch.Options.AlertSoundUnmute ) then
...
function me:Update ( ID, Name )
	if ( type( self.ID ) == "number" ) then -- Remove last overlay
			EbonSearch.Overlays.Remove( self.ID );
	end
	self.ID = ID;
...
function me:OnShow ()
	...
	self:EnableDrag( IsModifiedClick( "_EBONSEARCH_BUTTONDRAG" ) );
end
...
function me:OnHide ()
	...
	if ( self.HighlightedPlate and self.HighlightedPlate.EbonSearchHighlight ) then
		self.HighlightedPlate.EbonSearchHighlight:Hide();
