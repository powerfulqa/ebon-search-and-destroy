...
EbonSearch.Config.Search = me;

me.AddFoundCheckbox = CreateFrame( "CheckButton", "EbonSearchSearchAchievementAddFoundCheckbox", me, "InterfaceOptionsCheckButtonTemplate" );

me.TableContainer = CreateFrame( "Frame", nil, me );

me.NPCControls = CreateFrame( "Frame", nil, me.TableContainer );
me.NPCName = CreateFrame( "EditBox", "EbonSearchSearchNpcName", me.NPCControls, "InputBoxTemplate" );
me.NPCNpcID = CreateFrame( "EditBox", "EbonSearchSearchNpcID", me.NPCControls, "InputBoxTemplate" );
me.NPCWorld = CreateFrame( "EditBox", "EbonSearchSearchNpcWorld", me.NPCControls, "InputBoxTemplate" );
me.NPCWorldButton = CreateFrame( "Button", nil, me.NPCWorld );
me.NPCWorldButton.Dropdown = CreateFrame( "Frame", "EbonSearchSearchNPCWorldDropdown", me.NPCWorldButton );
...
function me.AddFoundCheckbox.setFunc ( Enable )
	if ( EbonSearch.SetAchievementsAddFound( Enable == "1" ) ) then
		EbonSearch.CacheListPrint( true );
	end
end
...
local function GetWorldID ( World )
	if ( World ~= "" ) then
		return EbonSearch.ContinentIDs[ World ] or World;
	end
end
...
function me:TabOnEnter ()
	...
	if ( self.AchievementID ) then
		...
		local WorldID = EbonSearch.Achievements[ self.AchievementID ].WorldID;
		...
		if ( not EbonSearch.OptionsCharacter.Achievements[ self.AchievementID ] ) then
...
function me:TabCheckOnClick ()
	...
	if ( not Enable ) then
		EbonSearch.AchievementRemove( AchievementID );
	elseif ( EbonSearch.AchievementAdd( AchievementID ) ) then -- Cache might have changed
		EbonSearch.CacheListPrint( true );
	end
end
...
function me.NPCAdd:OnClick ()
	local NpcID, Name, WorldID = me.NPCNpcID:GetNumber(), me.NPCName:GetText(), GetWorldID( me.NPCWorld:GetText() );
	if ( EbonSearch.TamableIDs[ NpcID ] ) then
		EbonSearch.Print( L.SEARCH_ADD_TAMABLE_FORMAT:format( Name ) );
	end
	EbonSearch.NPCRemove( NpcID );
	if ( EbonSearch.NPCAdd( NpcID, Name, WorldID ) ) then
		EbonSearch.CacheListPrint( true );
	end
	me.NPCClear();
end
...
function me.NPCRemove:OnClick ()
	EbonSearch.NPCRemove( me.NPCNpcID:GetNumber() );
	me.NPCClear();
end
...
function me.NPCWorldButton.Dropdown:initialize ()
	...
	if ( not EbonSearch.ContinentIDs[ CurrentWorld ] ) then -- Add current instance name
		-- Spacer
...
function me:AchievementUpdate ()
	local Achievement = EbonSearch.Achievements[ self.AchievementID ];
	for CriteriaID, NpcID in pairs( Achievement.Criteria ) do
		...
		if ( not EbonSearch.AchievementNPCIsActive( Achievement, NpcID ) ) then
			Row:SetAlpha( me.InactiveAlpha );
		end
	end
end
...
function me:AchievementActivate ()
	...
	me.Table.Header:SetAlpha( EbonSearch.OptionsCharacter.Achievements[ self.AchievementID ] and 1.0 or me.InactiveAlpha );
end
...
do
	...
	function me:TableCreate ()
		-- Note: Keep late bound so EbonOverlay can hook into the table as it's created
...
function me:default ()
	EbonSearch.Synchronize( EbonSearch.Options ); -- Resets only character settings
end
...
me.NPCAdd:SetScript( "OnEnter", EbonSearch.Config.ControlOnEnter );
...
local function EditBoxSetup ( self )
	...
	self:SetScript( "OnEnter", EbonSearch.Config.ControlOnEnter );
...
NpcID:SetMaxLetters( floor( log10( EbonSearch.NpcIDMax ) ) + 1 );
...
local function AddTab ( ID, Update, Activate, Deactivate )
	TabCount = TabCount + 1;
	local Tab = CreateFrame( "Button", "EbonSearchSearchTab"..TabCount, me.TableContainer, "TabButtonTemplate" );
	Tabs[ ID ] = Tab;
...
for AchievementID in pairs( EbonSearch.Achievements ) do
